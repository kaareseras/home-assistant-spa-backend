...
from custom_components.spa_backend.config_flow import (
...
def test_login_error_detail_uses_backend_detail():
    class Resp:
        status_code = 400
        text = '{"detail": "Email is not registered with us."}'

        def json(self):
            return {"detail": "Email is not registered with us."}

    err = requests.HTTPError(response=Resp())
    assert get_login_error_detail(err) == "Email is not registered with us."
